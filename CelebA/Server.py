...
class Server:
    ...
    def __round_train(self, clients):
        ...
        for k in range(m):
            ...
            totBalancing += clients[k].classes_balancing
            totSharpness += clients[k].imaage_sharpness
        ...
        for k in range(m):
            ...
            c4.append(clients[k].classes_balancing / totBalancing)
            c5.append(clients[k].imaage_sharpness / totSharpness)
        ...
        #         w = 0
        #         b = 0
        #         self.model.layers[layer].get_weights()[0] # to skip layers without weight before pop clients weights
        #         for k in range(m):
        #             (client_w, client_b) = clients[k].weights_list.pop(0)
        #             w += p[k] * client_w
        #             b += p[k] * client_b
